...
-- MAGIC %python
-- MAGIC events_stringsDF.describe()
...
-- MAGIC %python
-- MAGIC display(events_stringsDF
-- MAGIC     .where("value:ecommerce:purchase_revenue_in_usd = 1047.6")
-- MAGIC     .orderBy("key")
-- MAGIC     .limit(1)
-- MAGIC )
...
-- MAGIC data = [("Alice", [1, 2, 3]), ("Bob", [4, 5]), ("Charlie", [6])]
-- MAGIC df = spark.createDataFrame(data, ["name", "numbers"])
-- MAGIC # explode the numbers column
-- MAGIC df_exploded = df.select("name", explode("numbers").alias("number"))
-- MAGIC # show the result
-- MAGIC df_exploded.show()

-- COMMAND ----------

-- MAGIC %python
-- MAGIC from pyspark.sql.functions import explode, size
-- MAGIC display(df_exploded.where(size("numbers") > 1))
...
select collect_set(event_name) from exploded_events

-- COMMAND ----------

-- select collect_set(items.item_id) from exploded_events
-- -- -- select flatten(collect_set(items.item_id)) from exploded_events
select user_id, array_distinct(flatten(collect_set(items.item_id))) from exploded_events
group by user_id

-- COMMAND ----------

select * from exploded_events
where user_id = "UA000000106459577"
